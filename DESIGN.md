...
- `official` — **always `1.0` for every de jure official language**, no matter how many there are (it's a legal status, not a population measure — so it's never split across multiple official languages, e.g. South Africa's 11 official languages each get `1.0`, not `1/11`). It does *not* mean actual usage — that's captured by a separate `locallySpoken` entry for the same language. `scripts/validate.py` enforces this: any `official`-domain row with `speakersShare` other than `1.0` is a hard error, not just a style note — a bug found by external audit shipped a whole batch of dependent territories with population shares on their `official` rows before this check existed.
...
`name.local[]` (on both `country.yaml` and a region file) follows the same "official is a legal-status flag" spirit but isn't identical to it: at country level it's **at minimum** one entry per `languageUsage[]` row with `domain: official` — that part is a hard floor, not a maximum, so a file missing one of its own official languages there is a real `[data]` gap. At region level it's only the language(s) actually used/official **in that specific region**, which for a federal country's subdivisions is very often a strict subset of the country's full list (e.g. `CHE`'s German-speaking `CH-ZH` has one entry, not all four national languages) — except the `WHOLE` fallback, which gets the full country-level list since it represents the whole country generically. "Local" also isn't "official-only" **at either level**: `PYF`'s `country.yaml` and its `WHOLE` file both carry `fra` (official) **and** `tah` (not official, but genuinely, verifiably locally spoken) — a country-level entry for a disclosed, real local language beyond the official minimum is a legitimate addition, not a violation to strip out, as long as it's a genuinely-used local language and not guessed. (A past audit flagged this as ambiguous when three dependent territories carried a locally-relevant-but-non-official language — Danish in Greenland and the Faroe Islands, French in Guernsey — at country level; the resolution above is what settled it: keep it, it's the same pattern as `PYF`.) See `CONTRIBUTING.md`'s Hard rules for the enforced version of this.

**Every statistical entry must have a `sourceId` pointing to an authoritative source.** See "Source grounding" below.
